...
export function createFilter(): ProFilter {
  ...
  return {
    id: counter++,
    $inverted,
    toggleInvert: () => ($inverted.value = !$inverted.value),
    $highlighted,
    toggleHighlight: () => ($highlighted.value = !$highlighted.value),
    $searchScope,
    toggleSearchScope: () => ($searchScope.value = $searchScope.value === 'name' ? 'value' : 'name'),
  };
}
